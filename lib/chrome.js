    ...
    try {
      await runWithTimeout(async (timeoutInfo) => {
        try {
          ...
          if (chrome.viewport) { // support viewport
            chrome.viewport.width = parseFloat(chrome.viewport.width);
            chrome.viewport.height = parseFloat(chrome.viewport.height);
            chrome.viewport.deviceScaleFactor = parseFloat(chrome.viewport.deviceScaleFactor);
            page.setViewport(chrome.viewport);
          }

          if (chrome.scale) { // string to float .
            chrome.scale = parseFloat(chrome.scale);
          }
